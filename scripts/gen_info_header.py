...
def add_research_remapping(content: str):
    ...
    for race, remap in {
        "protoss": upgrade_map.PROTOSS_REMAP,
        "terran": upgrade_map.TERRAN_REMAP,
        "zerg": upgrade_map.ZERG_REMAP,
    }.items():
        content += (
            "const static std::unordered_map<int, std::array<int,3>>"
            f" {race}ResearchRemap = {{\n    "
        )
        for action, levels in remap.items():
            content += f"{{ {action}, {{{', '.join([str(l) for l in levels])}}} }}, "
        content += "};\n"
    return content
