...
class ReplayDatabase
{
  public:
     ...
     */
    template<typename T> [[maybe_unused]] auto addEntry(const T &data) -> bool;
     ...
     */
    template<typename T> [[nodiscard]] auto getEntry(std::size_t index) const -> T;
     ...
     */
    template<typename T> [[nodiscard]] auto getHashId(std::size_t index) const -> std::pair<std::string, std::uint32_t>;
     ...
     */
    template<typename T> [[nodiscard]] auto getHashes() const noexcept -> std::unordered_set<std::string>;
...
template<typename T> struct DatabaseIO
{
    using value_type = T;
